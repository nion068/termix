...
3.  **Operate**: Perform an action (e.g., yank a file with `y`).
...
3.  **Open**: Use `Enter` or `l` to open files in your default editor for content changes.
...
-   **Jump and Open**: `G → Enter` (open last file)
-   **Vim-style Dive**: `j → j → l → l` (down, down, enter, enter)
...
-   **Quick Yank**: `y → navigate → p`
-   **Yank Path**: `Y` (Shift + y) copies the absolute path to the system clipboard.
...
Select the `..` entry to see information about the parent directory, including its full path and last modified time.
...
Termix handles long filenames gracefully by truncating them with `...`, preserving the file extension so you can still identify the file type.
...
-   **Normal Mode**: Shows essential shortcuts and help prompt (`?`).
-   **Search Mode**: Shows your current query.
-   **Clipboard Active**: Shows what you've yanked/cut.
...
(Icons require a supported Nerd Font)
...
-   Review the full [Keyboard Shortcuts](./keyboard-shortcuts.md) reference.
