...
public class  FilePreviewService
{
    ...
    public IRenderable GetPreview(string? filePath, int verticalOffset, int horizontalOffset)
    {
        ...
        try
        {
            ...
            {
                return new Panel(Align.Center(new Text($"Image File\nPreview not supported yet."), VerticalAlignment.Middle))
        ...
        {
            return new Panel(Align.Center(new Text($"[red]Error reading file:[/] {ex.Message.EscapeMarkup()}"), VerticalAlignment.Middle))
                .Header(header).Expand().Border(BoxBorder.Rounded);
        }
    }

    private static bool IsImageFile(string extension) => SourceArray.Contains(extension, StringComparer.OrdinalIgnoreCase);
    private static bool IsBinary(byte[] fileBytes) => fileBytes.Take(8000).Any(b => b == 0);
}
